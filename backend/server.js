...
const { isConfigured: isMercadoPagoConfigured, createPreference: mpCreatePreference } = require('./services/mercadopago-client');
const helmet = require('helmet');
const compression = require('compression');
const { initDB, query, pool } = require('./database');
require('dotenv').config();

const { generateMonthlyInvoices, sendPaymentReminders, suspendOverdueTenants, suspendExpiredFreeTrials, backupDatabase } = require('./cron-billing');
const logger = require('./services/logger');
const morgan = require('morgan');
const { MP_CURRENCY, MP_LOCALE, MP_COUNTRY, PLANS, loadPlanPricesFromDB } = require('./services/payment-config');
...
// ========== ROUTES ==========
app.use('/api', require('./routes/auth')(loginLimiter, passwordResetLimiter));
app.use('/api', require('./routes/mercadopago')(createMercadoPagoPreference, MP_CURRENCY));
app.use('/api', require('./routes/tenant')(createMercadoPagoPreference, MP_CURRENCY, MP_LOCALE, MP_COUNTRY, PLANS));
app.use('/api', require('./routes/superadmin')(loginLimiter, createMercadoPagoPreference, MP_CURRENCY));
app.use('/p', require('./routes/public')(generateAvailableSlots, appointmentLimiter));
app.use('/api', require('./routes/misc')(apiLimiter));
...
// Rutas explícitas limpias (sin extensiones)
app.get('/', (req, res) => res.sendFile(path.join(frontendPublicPath, 'index.html')));
app.get('/landing', (req, res) => res.sendFile(path.join(frontendPublicPath, 'landing.html')));
app.get('/staff/login', (req, res) => res.sendFile(path.join(frontendPublicPath, 'staff', 'login.html')));
app.get('/staff/register', (req, res) => res.sendFile(path.join(frontendPublicPath, 'staff', 'register.html')));
app.get('/staff/dashboard', (req, res) => res.sendFile(path.join(frontendPublicPath, 'staff', 'dashboard.html')));
app.get('/staff/landing-editor', (req, res) => res.sendFile(path.join(frontendPublicPath, 'staff', 'landing-editor.html')));
app.get('/staff/forgot-password', (req, res) => res.sendFile(path.join(frontendPublicPath, 'staff', 'forgot-password.html')));
app.get('/staff/reset-password', (req, res) => res.sendFile(path.join(frontendPublicPath, 'staff', 'reset-password.html')));
app.get('/admin/login', (req, res) => res.sendFile(path.join(frontendPublicPath, 'admin', 'login.html')));
app.get('/admin/dashboard', (req, res) => res.sendFile(path.join(frontendPublicPath, 'admin', 'dashboard.html')));

// 404 - página personalizada
app.get('*', (req, res) => {
  if (!req.path.startsWith('/api') && !req.path.startsWith('/p/') && !req.path.startsWith('/uploads')) {
    res.status(404).sendFile(path.join(frontendPublicPath, '404.html'));
  }
});
