 ...
 */
module.exports = function (generateAvailableSlots, appointmentLimiter) {
  const router = Router();
  ...
  router.get('/:slug/availability', identifyTenant, async (req, res) => {
    try {
      ...
      const slots = generateAvailableSlots(date, service.duration, existing.rows, hoursConfig);
      res.json(slots);
    } catch (err) {
      logger.error(err);
  ...
  router.get('/:slug/staff/:staffId/availability', identifyTenant, async (req, res) => {
    try {
      ...
      let hoursConfig = staff.individual_hours || req.tenant.opening_hours;
      if (typeof hoursConfig === 'string') {
        try { hoursConfig = JSON.parse(hoursConfig); }
        catch (e) { hoursConfig = { startHour: 9, endHour: 19, workDays: [1, 2, 3, 4, 5] }; }
      }
