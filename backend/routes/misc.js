...
const { authenticate, authenticateStaff, checkTenantActive, checkTrialExpiration } = require('../middleware');
 ...
 */
module.exports = function (apiLimiter) {
  const router = Router();
  ...
  router.get('/agenda', authenticate(['client']), async (req, res) => {
    ...
    } catch (err) { logger.error('Error al cargar agenda', { error: err.message }); res.status(500).json({ error: 'Error al cargar agenda' }); }
  });
  ...
  router.post('/agenda', authenticate(['client']), async (req, res) => {
    ...
    } catch (err) { logger.error(err); res.status(500).json({ error: 'Error al crear evento' }); }
  });
