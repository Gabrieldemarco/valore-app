 ...
 */
module.exports = function (loginLimiter, createMercadoPagoPreference, MP_CURRENCY) {
  const router = Router();
  ...
  router.post('/super-admin/login', loginLimiter, async (req, res) => {
    ...
    } catch (err) { logger.error(err); res.status(500).json({ error: 'Error de autenticación' }); }
  });
  ...
  router.get('/super-admin/tenants', authenticateSuperAdmin, async (req, res) => {
    ...
    } catch (err) { logger.error(err); res.status(500).json({ error: 'Error al cargar peluquerías' }); }
  });
  ...
  router.post('/super-admin/tenants', authenticateSuperAdmin, async (req, res) => {
    ...
    } catch (err) { logger.error(err); res.status(500).json({ error: 'Error al crear peluquería' }); }
  });
  ...
  router.put('/super-admin/tenants/:id', authenticateSuperAdmin, async (req, res) => {
    ...
    } catch (err) { logger.error(err); res.status(500).json({ error: 'Error al actualizar' }); }
  });
  ...
  router.delete('/super-admin/tenants/:id', authenticateSuperAdmin, async (req, res) => {
    ...
    } catch (err) { logger.error(err); res.status(500).json({ error: 'Error al eliminar' }); }
  });
  ...
  router.get('/super-admin/tenants/:id', authenticateSuperAdmin, async (req, res) => {
    try {
      ...
      res.json({ tenant });
    } catch (err) { logger.error(err); res.status(500).json({ error: 'Error al cargar tenant' }); }
  });
  ...
  router.post('/super-admin/tenants/:id/reactivate', authenticateSuperAdmin, async (req, res) => {
    ...
    } catch (err) { logger.error(err); res.status(500).json({ error: 'Error al reactivar' }); }
  });
  ...
  router.get('/super-admin/tenants/:tenantId/invoices', authenticateSuperAdmin, async (req, res) => {
    ...
    } catch (err) { logger.error(err); res.status(500).json({ error: 'Error al cargar facturas' }); }
  });
  ...
  router.post('/super-admin/invoices', authenticateSuperAdmin, async (req, res) => {
    ...
    } catch (err) { logger.error(err); res.status(500).json({ error: 'Error al crear factura' }); }
  });
  ...
  router.put('/super-admin/invoices/:id/pay', authenticateSuperAdmin, async (req, res) => {
    ...
    } catch (err) { logger.error(err); res.status(500).json({ error: 'Error al actualizar factura' }); }
  });
  ...
  router.get('/super-admin/stats/billing', authenticateSuperAdmin, async (req, res) => {
    ...
    } catch (err) { logger.error(err); res.status(500).json({ error: 'Error al cargar estadísticas' }); }
  });
