...
const { PLANS, MP_CURRENCY } = require('./services/payment-config');
        ...
        for (const tenant of expiredTenants) {
            try {
                ...
                if (tenant.notification_email) {
                    const daysOverdue = Math.ceil((new Date() - new Date(tenant.trial_end_date)) / (1000 * 60 * 60 * 24));
...
async function sendPaymentReminders() {
    ...
    try {
        ...
        for (const inv of pendingInvoices) {
            ...
            const daysLeft = Math.ceil((new Date(inv.due_date) - now) / (1000 * 60 * 60 * 24));
...
async function suspendOverdueTenants() {
    ...
    try {
        ...
        for (const tenant of overdueTenants) {
            try {
                const daysOverdue = Math.floor((new Date() - new Date(tenant.oldest_due_date)) / (1000 * 60 * 60 * 24));
...
module.exports = {
    ...
    suspendExpiredFreeTrials,  // <-- ✅ AGREGAR ESTA LÍNEA
    backupDatabase,
    PLANS,
    MP_CURRENCY
};