...
interface Props {
  ...
  setPosPaymentMethod: (v: 'cash' | 'card' | 'mp') => void;
          ...
          <div className="grid-auto-fill-140 overflow-y-auto max-h-400">
            {productsList.filter(p => p.active && (p.stock < 1 || p.name.toLowerCase().includes(posSearch.toLowerCase()))).length === 0 && posSearch ? (
              ...
              <div key={p.id} onClick={() => p.stock > 0 && addToCart(p)} style={{
                ...
                <div className="font-600 text-border fs-15">{p.name}</div>
                <div className="font-700 text-primary mt-4 fs-17">${p.price}</div>
        ...
        <div className="flex-300">
          <div className="glass-panel p-16 mb-12">
                ...
                {posCart.map((item, idx) => (
                  <div key={idx} className="flex-center flex-gap-8" style={{ padding: '6px 0', borderBottom: '1px solid rgba(148,163,184,0.1)' }}>
                    ...
                    <div className="flex-center flex-gap-4">
                      <button className="dash-btn text-sm-2 px-8 py-2" onClick={() => updateCartQty(item.product_id, item.quantity - 1)}>-</button>
                      <span className="text-center minw-20">{item.quantity}</span>
                      <button className="dash-btn text-sm-2 px-8 py-2" onClick={() => updateCartQty(item.product_id, item.quantity + 1)}>+</button>
                    </div>
                    <div className="font-700 text-right minw-60">${item.total.toFixed(2)}</div>
                    <button className="dash-btn dash-btn-danger px-6 py-2 fs-11" onClick={() => removeFromCart(item.product_id)}>✕</button>
                  </div>
                ))}
              </div>
            )}
            <div className="flex-between mt-12 pt-12" style={{ borderTop: '1px solid rgba(148,163,184,0.2)' }}>
              ...
              <span className="font-700 text-primary fs-21">${posTotal.toFixed(2)}</span>
            </div>
          </div>
          ...
          <button className="btn btn-primary w-full p-14" onClick={checkout} disabled={posCart.length === 0 || posCheckoutLoading}>
